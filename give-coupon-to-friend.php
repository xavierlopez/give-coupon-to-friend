 ...
 * Plugin URI: https://kuestiona.com
...
function register_my_custom_submenu_page() {
	...
	add_action( 'admin_init', 'gctf_register_plugin_settings' );
}
...
*/
add_action( 'woocommerce_email_before_order_table', 'gctf_add_content_specific_email', 20, 4 );
function gctf_add_content_specific_email( $order, $sent_to_admin, $plain_text, $email ) {
    ...
    if ( $email->id == 'customer_completed_order' &&  get_post_meta($order->get_id(),'cupon_generado') )  {
        echo "<div id='cupon_email'>".get_option('gctf_text_coupon','Give this coupon to a friend:');
